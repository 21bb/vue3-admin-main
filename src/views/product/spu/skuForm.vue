    ...
    <el-form-item label="平台属性">
      ...
      <div class="inline-flex flex-wrap gap-10">
        <el-form-item v-for="(item) in attrArr" :key="item.id" :label="item.attrName">
          <!-- 必须加 v-model -->
          <el-select v-model="item.attrIdAndValueId" style="width: 200px" placeholder="请选择">
            <!-- 内层循环不能用 index，改成 i -->
            <el-option :value="`${item.id}:${attrValue.id}`" v-for="(attrValue, i) in item.attrValueList" :key="i"
    ...
    <el-form-item label="销售属性">
      <div class="inline-flex flex-wrap gap-10">
        <el-form-item v-for="(item) in saleArr" :key="item.id" :label="item.saleAttrName">
          <!-- 必须加 v-model -->
          <el-select v-model="item.saleIdAndValueId" style="width: 200px" placeholder="请选择">

            <!-- 内层循环不能用 index，改成 i -->
            <el-option :value="`${item.id}:${saleAttrValue.id}`" v-for="(saleAttrValue, i) in item.spuSaleAttrValueList"
