<template>
  <el-form label-width="100px">
    ...
    <el-form-item label="SPU图标 ">
      ...
      <el-upload v-model:file-list="imgList" action="http://192.168.10.130:10086/api/admin/product/fileUpload"
    ...
    <el-form-item label="SPU销售属性">
      <!--展示销售属性的下拉菜单-->
      <el-select v-model="saleAttrIdAndValueName" style="width: 150px;"
      ...
      <el-table border style="margin: 10px 0px;" :data="saleAttr">
        ...
        <el-table-column label="销售属性值">
          <template #="{ row, $index }">
            <el-tag closable style="margin:0px 5px" @close="row.spuSaleAttrValueList.splice($index, 1)"
              v-for="(item, $index) in row.spuSaleAttrValueList" :key="item.id" class="mx-1">
...
import type { saleAttrValue, SaleAttr, SaleAttrResponseData, SpuData } from '@/api/product/spu/type';
import { reqAllTradeMark, reqSpuImageList, reqSpuHasSaleAttr, reqAllSaleAttr, reqAddOrUpdateSpu } from '@/api/product/spu';
import type { AllTradeMark, SpuHasImg, HasSaleAttrResponseData, SpuImg, HasSaleAttr } from '@/api/product/spu/type';
...
//存储已有的SPU对象
let SpuParams = ref<SpuData | null>({
...
//保存按钮的回调
const save = async () => {
  SpuParams.value.tmId = Number(SpuParams.value.tmId)
  //整理参数
  //1：照片墙的数据
